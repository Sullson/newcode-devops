...
A managed Kubernetes cluster whose API server is **private**. The control plane is reachable only
...
The control plane stays private, the kubeconfig points only at the tailnet proxy, and the CI node disappears
...
ClusterIP Service. Ingress is outbound-only, so nothing on the cluster listens to the internet. The
...
endpoints stay private, behind the same tunnel. Model weights and provider API keys would flow through the same
